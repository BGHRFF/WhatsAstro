...
Astro.addCommand({pattern: 'update$', fromMe: true, desc: Lang.UPDATER_DESC}, (async (message, match) => {
    await git.fetch();
...
Astro.addCommand({pattern: 'update now$', fromMe: true, desc: Lang.UPDATE_NOW_DESC, dontAddCommandList: true}, (async (message, match) => {
    await git.fetch();
    var commits = await git.log([Config.BRANCH + '..origin/' + Config.BRANCH]);
    if (commits.total === 0) {
        ...
        if (Config.HEROKU.HEROKU) {
            ...
            git.pull((async (err, update) => {
                if(update && update.summary.changes) {
                    ...
                    await message.client.sendMessage(
                        message.jid,'*❌ Güncəlləmə uğursuz oldu!*\n*Xəta:* ```' + err + '```', MessageType.text);
                }
            }));
